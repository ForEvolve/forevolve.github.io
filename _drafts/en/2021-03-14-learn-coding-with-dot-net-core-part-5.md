...
This short article is the foundation of more dynamic notions enabling our programs to change and react based on simple user interactions.
...
In our case, we will continue to use console applications because their are the simplest.
...
The first two can be used for more complex scenarios while the third one is very straight forward.

As its name implies, `Console.ReadLine` reads the line entered by the user.
This is the method we are focusing on in this article.
...
The `Console.ReadLine` method return a string, which represent the line written in the console by the user.
Here is an example:
...
At this point, it will resume and continue, write the read line back to the console.

Here is the final console content if I write `Hello Amigo!` then press the `enter` key:
...
Next, we look at the linear flow of execution of our program.
...
1. The program executes the right-end of the assignation operator (`=`), the `Console.ReadLine()` method, which waits for the user to hit the `<ENTER>` key.
    > The assignation operator `=` is always the last to be executed, it has the lowest priority.
...
![Code sequence graph](//cdn.forevolve.com/blog/images/2021/2021-03-00-learn-to-code-input-graph-4.png)

In this case, the `Console.ReadLine()` method manages the bifurcation even if code-wise, the flow is linear.
We will learn some ways to control the flow of a program in future articles.
...
Then, the program must great that user, using the following format: `Greetings Carl-Hugo Marcotte!`.

> `Carl-Hugo` was the first name and `Marcotte` the last name entered by the user.
...
The code will include `Console.Title`.
...
![Adding Console.Clear calls](//cdn.forevolve.com/blog/images/2021/2021-03-00-learn-to-code-input-ConsoleClear.gif)
