...
Our Clan sub-system is pretty basic indeed, but it allowed us to learn the patterns without bothering too much with external dependencies.
...
### INinjaService
The ninja service should give us access to the Ninja entities. Once again, it will only support CRUD operations.
...
### INinjaRepository
The `INinjaRepository` look the same as the `INinjaService`.
Again, we are building a simple data-driven CRUD Web API.
...
<aside>
    ...
    <p>
        As you may have noticed, I decorated the actions with <code>ProducesResponseType</code> attributes.
        ...
        For now, it is only useful for the humans programming the APIs, but we could create autogenerated documentation of our APIs and auto-generated REST clients.
    </p>
    <p>
        ...
        Swashbuckle would take the <code>ProducesResponseType</code> attributes and turn them into Swagger Specifications for you.
    </p>
    <p>
        See <a href="https://github.com/Azure/autorest">AutoRest</a> to generate a REST client automatically, based on that Swagger specifications.
    </p>
    <footer>
        <figure>
            <blockquote>
            ...
            <figcaption>
                Definition from the official <a href="https://swagger.io/">Swagger</a> home page.
            </figcaption>
        </figure>
    </footer>
</aside>
...
Instead of using built-in ones or returning `null` we will be able to catch `NinjaApiException`s.

On top of `NinjaApiException`, we will also create `ClanNotFoundException` and `NinjaNotFoundException`, as follow:
...
>
> As for the other two, `catch (ClanNotFoundException)` is easier to understand that `catch (ArgumentException)` or `if (someObject == null)`.
